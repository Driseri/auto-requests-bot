...
- [Индексация вручную внесённых строк](#индексация-вручную-внесённых-строк)
...
2. Выполните `/start`.
3. Создайте тестовую одиночную заявку и проверьте, что бот вернул её ID.
4. Убедитесь, что строка с тем же ID появилась в правильной Google-таблице.
5. Установите статус `В работе` и дождитесь личного уведомления автора.
...
### Уведомления общего чата редакторов
...
Тот же чат получает durable-уведомление `editor-scriptwriter-response`, когда поле
`Ответ сценариста` любой отслеживаемой заявки стабильно заполнено три polling-цикла.
Уведомление работает для раскаток, срочных и интеграций, включая ADD, EDIT и CHIPS;
статус заявки не является условием. Регистрация нескольких срочных строк нового
массового резерва создаёт одно агрегированное событие
`urgent-editor-bulk-reservation-created`. Переменная
`URGENT_EDITOR_NOTIFICATIONS_ENABLED` сохраняет старое имя для совместимости и
включает все перечисленные уведомления общего чата.

При первом запуске версии с поддержкой всех типов текущие значения поля `Ответ
сценариста` сохраняются как baseline без ретроспективных сообщений. Поэтому чат
получает только ответы, появившиеся или изменившиеся после обновления.
...
2. Пройти создание тестовой одиночной заявки и проверить ID в ответе бота.
3. Проверить, что строка с тем же ID появилась в нужной Google-таблице.
4. Установить статус `В работе` и дождаться уведомления автора.
...
## Индексация вручную внесённых строк

Используйте этот раздел, если строка заявки есть в Google Sheets, но была перенесена
или внесена вручную и у неё пустые технические поля: `ID заявки`, `Тип заявки`,
`Дата заявки`, `Направление`, `Тип ответа`, `Telegram ID`, `Тип изменения`.

Это редкий административный инструмент. Он не должен заменять обычное заведение
заявок через бота.

### 1. Dry-run

Команда только читает Google Sheets и SQLite:
...
Зачем: увидеть безопасных кандидатов и строки, которые инструмент пропустит.
Кандидатом считается только строка внутри распознанной ADD/EDIT или CHIPS-секции,
где заполнены обязательные пользовательские поля, но пустой `ID заявки` и остальные
технические поля.

### 2. Выполнить индексацию выбранных строк

Перед `--execute` желательно сделать backup SQLite. Запускать запись нужно только
по явно проверенным строкам:
...
Если реальный `telegram_user_id` неизвестен, можно явно добавить
`--allow-unknown-user`, но это крайний режим: личные уведомления по такой заявке не
будут корректно доставляться пользователю. Для production предпочтительно найти
пользователя и указать настоящий `--telegram-user-id`.

### 3. Ограничить область поиска

Для больших листов используйте фильтры:

```bash
docker compose -f docker-compose.prod.yml exec -T bot python -m app.sheet_indexer \
  --spreadsheet-id "<spreadsheet_id>" \
  --sheet-name "Интеграции" \
  --from-row 20 \
  --to-row 80 \
  --change-type ADD
```
...
Строка даты должна оставаться отдельной видимой строкой: бот группирует только
строки заявок под датой, а не сам разделитель дня. Если после ручной правки листа
сворачивание захватывает строку следующей даты, сначала проверьте формат дат:
они должны отображаться как `ДД.ММ.ГГ`.
Миграция перестраивает порядок строк листа, поэтому выполняйте её только в
maintenance window: остановить `bot`, сделать backup SQLite, выполнить dry-run,
затем `--execute` и после проверки запустить `bot` обратно.
...
Штатный способ удалить ошибочную одиночную заявку из рабочей таблицы — поставить ей статус `Удаление`.
Бот удалит строку примерно после двух polling-циклов, если статус останется неизменным, а `ID заявки`
в строке совпадет с tracking в SQLite. Если статус поменять обратно до второго цикла, удаление отменится.
Если несколько строк на одном листе одновременно помечены на удаление, бот удаляет их снизу вверх,
чтобы удаление верхней строки не сдвинуло координаты нижних заявок до их проверки.

Ручные SQL-команды ниже нужны только для аварийной чистки tracking, когда строка уже удалена вручную из
Google Sheets или требуется убрать тестовые/ошибочные записи из SQLite без изменения таблиц.
