...
| `trigger` | `string` | нет | Для V6.1 — `create`; `edit` встречается только в исторических событиях старого flow. |
| `prompt_version` | `string` | нет | Версия из имени system prompt, например `v6.1`; для совместимого тестового клиента `unknown`. |
...
| `blocking_rule` | `string` | да | Совместимый код `1.1`, `1.2`, `2.1`, `3.1` либо `null`; для `missing_change_action` всегда `null`. |
| `gap_code` | `string` | да | Один из пяти кодов пробела V6.1 либо `null`. |
...
Совместимое краткое событие создаётся после получения дополнения, присоединения
его к сохранённой версии поля «Суть изменений» с новой строки и до второго
вызова GigaChat.
...
| `clarification_number` | `integer` | Совместимый номер повторной проверки, для V6.1 всегда `1`. |
| `trigger` | `string` | Для V6.1 всегда `create`. |
...
исходную и актуальную версии поля, до двух элементов `iterations`, сырой
`missing_detail`, сырой и структурированный ответы, prompt/model telemetry,
технические ошибки и действия
...
Полная структура колонок, вложенных объектов, состояний и итогов описана в
[справочнике `llm_recommendation_processes`](LLM_RECOMMENDATION_PROCESS_JSON.md).
...
- `llm_check_completed` встречается не более двух раз для V6.1+: первоначальная проверка и проверка объединённого текста после дополнения; review-edit новую строку не создаёт;
