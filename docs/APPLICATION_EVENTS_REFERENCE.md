...
заявки через GigaChat: первоначальной и повторной после уточнения. Проверка после
редактирования создаёт отдельное событие. Внутренние HTTP-повторы не создают
отдельные строки.
...
| `trigger` | `string` | нет | `create` или `edit`. |
| `prompt_version` | `string` | нет | Версия из имени system prompt, например `v3`; для совместимого тестового клиента `unknown`. |
...
Создаётся после получения ответа пользователя на уточнение и до повторного вызова
GigaChat.
...
| `clarification_number` | `integer` | Порядковый номер уточнения в текущем черновике, начиная с 1. |
| `trigger` | `string` | `create` или `edit`. |

Текст, preview и длина уточнения не сохраняются. Из-за порядка записи возможно
...
Типичный ADD/EDIT без уточнения:
...
| `llm_check_completed` | Одна транзакция с обновлением `draft.llm_check_status`, результата и счётчика уточнений. |
...
- `llm_check_completed` повторяется для уточнений и редактирования;
- количество заявок считается по уникальному `application_id`, а не по числу всех
  событий.

## 7. Данные и доступ

LLM-телеметрия спроектирована без пользовательского содержимого. Однако текущие
polling-события содержат полные рабочие значения:
