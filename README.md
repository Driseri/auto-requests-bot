...
6. Бот вставляет ровно это количество пустых строк в конец нужной рабочей секции и выделяет резерв толстыми верхней и нижней границами.
7. Пользователь заполняет строки в обычных колонках выбранной схемы.
8. Пользователь нажимает `Заявка заполнена`, после чего бот регистрирует заполненные строки как обычные одиночные заявки, выдает `ID заявки`, ставит строковый статус `Новая` и оставляет `ID пачки` пустым.
...
Если цельный блок массовой заявки перемещен внутри исходного физического листа, бот ищет строку-шапку по `ID пачки`, обновляет координаты пачки и всех ее заявок в SQLite и продолжает polling по новому диапазону. Переименование листа поддерживается через стабильный Google `sheet_id`. Перенос отдельных строк, перенос на другой лист и неоднозначные копии одного `ID пачки` автоматически не перепривязываются. После неудачного поиска следующая полная проверка выполняется через `BULK_RELOCATION_SEARCH_INTERVAL_SECONDS`.
...
Массовые заявки, включая строки с типом изменения `CHIPS`, используют прежнюю массовую схему и этим изменением не затрагиваются.
