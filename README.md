...
- до среды 14:00 по времени `BOT_TIMEZONE` - в лист среды следующей недели, например `08.06 ср`;
...
Новая секция CHIPS в недельной вкладке содержит рабочие колонки `A:K`: `Закрепленный сценарист`, `Интент`, `Причина`, три текстовых фрагмента, `Комментарий качества`, `Вопросы/комментарии редактора`, `Ответ сценариста`, `Статус`, `Редактор`. Служебные поля находятся в `L:U`; удалять их нельзя. Dropdown статуса и редактора применяется к `J:K`.

Если старая секция `CHIPS` пуста, бот заменяет только её заголовок новой схемой. Если в ней уже есть заявки, строки и заголовок сохраняются, а бот создаёт рядом секцию `CHIPS V2` и направляет новые заявки туда. Старые и новые секции продолжают участвовать в polling. Для CHIPS бот уведомляет об изменениях важных статусов и добавляет комментарий при статусе `Нужны пояснения`, но не ожидает и не отправляет итоговый ответ редактора.
